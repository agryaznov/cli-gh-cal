...
function CliGhCal(data, options) {
    ...
    if (data.length) {
        ...
        options.end = Deffy(options.end, now);
    }

    var cropped = false;

    var records_num = Object.keys(cal.days).length;
    var weeks = now.diff(options.start, "weeks");
    cal.weekly_avg = (cal.total/weeks).toFixed(0);
    cal.times_a_week = (records_num/weeks).toFixed(0);
    
    if (!options.noCrop) {
	var weeks_total = options.end.diff(options.start, "weeks");
        var diff = Math.ceil(CLI_COLUMNS - (weeks_total * 2 + 11) - 1);
        if (diff < 0) {
            cropped = true;
    ...
    if (CLI_COLUMNS > 45) {
        strYear +=
            ...
            + "\n" + when + " | Total: " + cal.total + " (" + records_num + " records)"
	    +  (cropped ? "\n" : " | ") + "Max per record: " + cal.max
       //   + (cropped ? "\n" : " | ") + "Longest Streak: " + cal.lStreak + " days"
          + (cropped ? "\n" : " | ") + "Weekly Avg: " + cal.weekly_avg + " (" + cal.times_a_week + " times a week)"
          ;
    }
